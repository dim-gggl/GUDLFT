...
def test_find_club_by_unknown_email():
    ...
    """
    clubs = [
        {"name": "Test Club", "email": "test@test.com"},
...
def test_update_existing_club_and_competition():
    ...
    with patch.object(data_manager, 'CLUBS', original_clubs), patch.object(
        data_manager, 'COMPETITIONS', original_competitions
    ):
...
def test_successful_booking(app_context):
    ...
    with patch(
        'data_manager.validate_places_required', return_value=None
    ), patch(
        'data_manager.validate_competition_date', return_value=None
    ), patch(
        'data_manager.update_clubs_and_competitions'
    ) as mock_update, patch(
        'data_manager.save_clubs_and_competitions'
    ) as mock_save:
