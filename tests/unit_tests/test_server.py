...
def mock_json_functions():
    ...
    with patch('server.CLUBS', clubs_data), patch(
        'server.COMPETITIONS', competitions_data
    ):
        yield
...
class TestSummaryRoute:
    ...
    def test_show_summary_with_invalid_email(
        self, test_app, mock_json_functions
    ):
...
class TestPurchasePlacesRoute:
    ...
    def test_purchase_places_success(self, test_app, mock_json_functions):
        ...
        with patch(
            'server.update_data_after_booking', return_value=None
    ...
    def test_purchase_places_with_error(self, test_app, mock_json_functions):
        ...
        with patch(
            'server.update_data_after_booking', return_value="Error message"
        ), patch('server.get_obj_by_field') as mock_get:
            mock_get.side_effect = [
                {
                    "name": "Spring Festival",
