...
class MockDataManager:
    ...
    def get_club_by_name(self, name):
        ...
        return next(
            (club for club in self.stored_clubs if club["name"] == name), None
        )

    def get_competition_by_name(self, name):
        """Get a competition by name from stored data"""
        return next(
            (c for c in self.stored_competitions if c["name"] == name), None
        )
...
def mock_json_functions(mock_data_manager):
    ...
    with patch(
        'data_manager.load_data', side_effect=mock_data_manager.load_data
    ), patch(
        'data_manager.save_json', side_effect=mock_data_manager.save_json
    ), patch(
        'server.CLUBS', mock_data_manager.stored_clubs
    ), patch(
        'server.COMPETITIONS', mock_data_manager.stored_competitions
    ), patch(
        'data_manager.CLUBS', mock_data_manager.stored_clubs
    ), patch(
        'data_manager.COMPETITIONS', mock_data_manager.stored_competitions
    ):
