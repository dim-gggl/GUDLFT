...
def test_club_points_updated_after_purchase(test_app, mock_json_functions):
    ...
    initial_points = mock_json_functions.get_club_by_name("Simply Lift")[
        "points"
    ]
    ...
    assert (
        int(updated_competition["number_of_places"]) == int(initial_places) - 1
    )
...
def test_clubs_cannot_book_past_competitions(test_app, mock_json_functions):
    ...
    with test_app.test_client() as client:
        ...
        competition = mock_json_functions.get_competition_by_name(
            "Fall Classic"
        )
        if competition["date"] < datetime.now().isoformat():
            assert "You cannot book past competitions" in response.data.decode(
                "utf-8"
            )
        else:
            assert response.status_code == 200
